...
function showToolMenu(menu){
	...
	case "edit":		// 编辑菜单
		...
		break;
	case "object":		// 对象效果菜单
		sMenu += getMenuRow("", "ShowDialog('dialog/selcolor.htm?action=bgcolor', 280, 270, true)", "bgcolor.gif", "对象背景颜色");
		sMenu += getMenuRow("", "ShowDialog('dialog/backimage.htm', 350, 230, true)", "bgpic.gif", "背景图片");
		sMenu += sMenuHr;
		...
		break;
	case "component":	// 组件菜单
		sMenu += getMenuRow("", "ShowDialog('dialog/img.htm', 360, 330, true)", "img.gif", "插入或修改图片");
		sMenu += getMenuRow("", "ShowDialog('dialog/flash.htm', 350, 220, true)", "flash.gif", "插入Flash动画");
		sMenu += getMenuRow("", "ShowDialog('dialog/media.htm', 350, 220, true)", "media.gif", "插入自动播放媒体");
		sMenu += getMenuRow("", "ShowDialog('dialog/file.htm', 350, 170, true)", "file.gif", "插入其他文件");
		sMenu += sMenuHr;
