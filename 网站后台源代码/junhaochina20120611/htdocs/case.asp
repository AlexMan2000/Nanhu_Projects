...
<div id="Main_right">
  ...
  <%	
		...
		str="select * from picturesort1 where piccat_id1="&piccat_id&" order by piccat_px asc,id desc"
  ...
  %>
    <li>
      <div class="case_list1"><a href="case1.asp?cat_id=<%=cat_id%>&piccat_id=<%=piccat_id%>&piccat_id1=<%=rs("id")%>"><img src="jh_admin/<%=rs("piccat_pic")%>" width="144" height="107"/></a></div>
      <div><a href="case1.asp"><%=rs("piccat_name1")%></a></div>
    </li>
    <% 
							if cnt < iCount then 
								rs.movenext
  ...
  <div class="yema">
    <%
				...
				Sub PageControl(iCount,pagecount,page,table_style,font_style)'生成上一页下一页链接
    				...
    				if page<=1 then
       					 Response.Write ("<font class='yema1'>首页</font>")        
       		   			 Response.Write ("<font class='yema1'>上一页</font>")
    				else        
        				Response.Write("<A class='yema1' HREF=" & action & "?" & temp & "Page=1 >首页</A>&nbsp;")
        				Response.Write("<A class='yema1' HREF=" & action & "?" & temp & "Page=" & (Page-1) & ">上一页</A>&nbsp;")
    				end if

    				if page>=pagecount then
						Response.Write ("<font class='yema1'>下一页</font>")
        				Response.Write ("<font class='yema1'>末页</font>")         
    				else
        				Response.Write("<A class='yema1' HREF=" & action & "?" & temp & "Page=" & (Page+1) & ">下一页</A></font>")
        				Response.Write("<A class='yema1' HREF=" & action & "?" & temp & "Page=" & pagecount & ">末页</A></font>")            
    				end if
